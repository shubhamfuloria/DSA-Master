...
string lpath = "";
int lwt = INT_MIN;
void longestPath(vector<Edge> graph[], int src, int dest, vector<bool> &visited, string psf, int wsf)
{
  if (src == dest)
  {
    if (lwt < wsf)
  ...
  for (auto edge : graph[src])
  {
    ...
    {
      longestPath(graph, edge.nbr, dest, visited, psf + to_string(edge.nbr), wsf + edge.wt);
    }
  }
...
int main()
{
  ...
  longestPath(graph, src, dest, visited, to_string(src), 0);
  cout << lpath << "\t" << lwt << endl;
}